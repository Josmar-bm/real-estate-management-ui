...
export default function RegisterPage() {
  ...
  const canSubmit = useMemo(() => {
    return name.trim().length > 1 && email.trim().length > 3 && password.length >= 6 && passwordsMatch && !isSubmitting;
  }, [name, email, password, passwordsMatch, isSubmitting]);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(
        payload.user?.email
          ? `Account created for ${name}. You can now continue to login.`
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(145deg,#ecfeff_0%,#ecfdf5_50%,#fff7ed_100%)] px-6 py-10 text-zinc-900">
      <section className="mx-auto flex w-full max-w-6xl items-center justify-center">
        <div className="jbm-fade-up grid w-full overflow-hidden rounded-3xl border border-zinc-200/70 bg-white/85 shadow-[0_20px_70px_-35px_rgba(15,23,42,0.5)] backdrop-blur-sm md:grid-cols-[1.15fr_1fr]">
          <div className="relative hidden bg-zinc-950 p-10 text-zinc-100 md:block">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,rgba(20,184,166,0.45),transparent_42%),radial-gradient(circle_at_80%_78%,rgba(245,158,11,0.35),transparent_40%)]" />
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div>
                ...
                <h1 className="mt-4 text-3xl font-semibold leading-tight">
                  Build your account and launch your property workspace.
                </h1>
              </div>
              <p className="max-w-xs text-sm text-zinc-300">
                Connect agents, listings, and daily operations from one secure platform.
              </p>
            </div>
          </div>
          ...
          <div className="p-8 sm:p-10">
            ...
            <p className="mt-6 text-sm text-zinc-600">
              ...
              <Link className="font-medium text-zinc-900 underline decoration-cyan-500 underline-offset-4" href="/login">
                Sign in
              </Link>
              .
            </p>
